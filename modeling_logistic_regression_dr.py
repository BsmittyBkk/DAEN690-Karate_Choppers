...
# list of attributes to use for dynamic rollover models after feature selection
df_dr = df[['Roll Acceleration','Pitch Acceleration','Yaw Acceleration','Roll','Roll Rate','Pitch Rate','Groundspeed','Wind Speed(True)',
      'Wind Direction(Mag)','Gross Weight','Fuel Weight','Dynamic Rollover']]
...
imb_set_dr = df_dr.drop(columns=exclude_dependent_cols)
...
imb_set_dr_y = df_dr.loc[:, 'Dynamic Rollover']
...
plt.title('Top 20 Variable Importance Plot : Dynamic Rollover Logistic Regression Model')
plt.tight_layout()
plt.show()
